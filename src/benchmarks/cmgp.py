# COPIED FROM: https://github.com/vanderschaarlab/mlforhealthlabpub/blob/main/alg/causal_multitask_gaussian_processes_ite
...
class CMGP:
    ...
    def _fit(
        ...
        try:
            ...
            use_gpu = True
            logging.info("GMGP - GPU is available; will use GPU for training")
        except ImportError:
            use_gpu = False
            logging.info("GMGP - GPU is not available; will use CPU for training")
        K0 = GPy.kern.RBF(self.dim, ARD=True, useGPU=use_gpu)
        K1 = GPy.kern.RBF(self.dim, ARD=True, useGPU=use_gpu)
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.dim == 1:
            ...
            X_1_shape = X_1.shape
            noise_dict_0 = {
                "output_index": X_0[:, X_0_shape[1] - 1].reshape((X_0_shape[0], 1)).astype(int)
            }
            noise_dict_1 = {
                "output_index": X_1[:, X_1_shape[1] - 1].reshape((X_1_shape[0], 1)).astype(int)
            }
    ...
    def _initialize_hyperparameters(self, X: np.ndarray, T: np.ndarray, Y: np.ndarray) -> None:
        """
        ...
        b0 = np.mean(
            (Dataset["Yk0"] - np.mean(Dataset["Yk0"])) * (Dataset["Yk1"] - np.mean(Dataset["Yk1"]))
        ) / (a0 * a1)
        b1 = b0
    ...
    def evaluate(self, X, Y0, Y1, alpha=0.05, return_p_values=False):
        ...
        """
        X0 = np.hstack([X, np.zeros_like(X[:, 1].reshape((len(X[:, 1]), 1)))])
        X1 = np.hstack([X, np.ones_like(X[:, 1].reshape((len(X[:, 1]), 1)))])
        noise_dict_0 = {
            "output_index": X0[:, X0.shape[1] - 1].reshape((X0.shape[0], 1)).astype(int)
        }
        noise_dict_1 = {
            "output_index": X1[:, X1.shape[1] - 1].reshape((X1.shape[0], 1)).astype(int)
        }
        mu0, var0 = self.model.predict(X0, Y_metadata=noise_dict_0)
        mu1, var1 = self.model.predict(X1, Y_metadata=noise_dict_1)

        mu_ite = mu1 - mu0
        var_ite = var0 + var1  # Assuming independence
        ...
        # Interval prediction
        y0_upper = mu0 + np.sqrt(var0) * scipy.stats.norm.ppf(1 - alpha / 2)
        y0_lower = mu0 - np.sqrt(var0) * scipy.stats.norm.ppf(1 - alpha / 2)
        y1_upper = mu1 + np.sqrt(var1) * scipy.stats.norm.ppf(1 - alpha / 2)
        y1_lower = mu1 - np.sqrt(var1) * scipy.stats.norm.ppf(1 - alpha / 2)
        ite_upper = mu_ite + np.sqrt(var_ite) * scipy.stats.norm.ppf(1 - alpha / 2)
        ite_lower = mu_ite - np.sqrt(var_ite) * scipy.stats.norm.ppf(1 - alpha / 2)
        ...
        crps_y0 = crps_normal(Y0, mu=mu0, sigma=np.sqrt(var0), return_average=True)
        crps_y1 = crps_normal(Y1, mu=mu1, sigma=np.sqrt(var1), return_average=True)
        crps_ite = crps_normal(Y1 - Y0, mu=mu_ite, sigma=np.sqrt(var_ite), return_average=True)

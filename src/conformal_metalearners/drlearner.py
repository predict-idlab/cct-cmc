...
class conformalMetalearner:
    ...
    def __init__(
        ...
        self.models_0 = [
            base_learners_dict[self.base_learner](n_estimators=n_estimators_nuisance)
        ...
        if self.quantile_regression:
            base_args_u = dict(
                {"loss": "quantile", "alpha": 1 - (alpha_ / 2), "n_estimators": n_estimators_target}
            )
            base_args_l = dict(
                {"loss": "quantile", "alpha": alpha_ / 2, "n_estimators": n_estimators_target}
            )

            self.models_u = [
                base_learners_dict[self.base_learner](**base_args_u) for _ in range(self.n_folds)
            ]
            self.models_l = [
                base_learners_dict[self.base_learner](**base_args_l) for _ in range(self.n_folds)
            ]

        else:
            base_args_m = dict({"n_estimators": n_estimators})
            self.models_m = [
                base_learners_dict[self.base_learner](**base_args_m) for _ in range(self.n_folds)
            ]
    ...
    def get_pseudo_outcomes(self, W, pscores, Y, Y_hat_0, Y_hat_1, metalearner="DR"):
        ...
        if metalearner == "DR":
            ...
            Y_pseudo = (
                (W.reshape((-1, 1)) - pscores.reshape((-1, 1)))
                / (pscores.reshape((-1, 1)) * (1 - pscores.reshape((-1, 1))))
            ) * (Y.reshape((-1, 1)) - Y_hat.reshape((-1, 1))) + (
                Y_hat_1.reshape((-1, 1)) - Y_hat_0.reshape((-1, 1))
            )
    ...
    def fit(self, X, W, Y, pscores):
        ...
        for i, (train_index, test_index) in enumerate(self.skf.split(W, W)):
            X_1, W_1, Y_1, pscores_1 = (
                X[train_index, :],
                W[train_index],
                Y[train_index],
                pscores[train_index],
            )
            X_2, W_2, Y_2, pscores_2 = (
                X[test_index, :],
                W[test_index],
                Y[test_index],
                pscores[test_index],
            ...
            Y_pseudo = self.get_pseudo_outcomes(
                W_2, pscores_2, Y_2, Y_hat_0, Y_hat_1, self.metalearner
            ).reshape((-1,))
    ...
    def conformalize(self, alpha, X_calib, W_calib, Y_calib, pscores_calib, oracle=None):
        ...
        Y_DR_calib = self.get_pseudo_outcomes(
            W_calib, pscores_calib, Y_calib, y_hat_0_calibs, y_hat_1_calibs, self.metalearner
        )

        if self.quantile_regression:
            self.residuals = np.maximum(
                T_hat_DR_calib_l.reshape((-1, 1)) - Y_DR_calib,
                Y_DR_calib - T_hat_DR_calib_u.reshape((-1, 1)),
            )
        ...
        if oracle is not None:
            if self.quantile_regression:
                self.oracle_residuals = np.maximum(
                    T_hat_DR_calib_l.reshape((-1, 1)) - oracle.reshape((-1, 1)),
                    oracle.reshape((-1, 1)) - T_hat_DR_calib_u.reshape((-1, 1)),
                )

            else:
                self.oracle_residuals = np.abs(
                    T_hat_DR_calib.reshape((-1, 1)) - oracle.reshape((-1, 1)),
                )

        self.offset = np.sort(self.residuals, axis=0)[
            int(np.ceil((1 - alpha) * (1 + 1 / len(W_calib)) * len(W_calib)))
        ]

# Copied from: https://github.com/AlaaLab/conformal-metalearners/blob/main/models/metalearners.py
...
def conformal_metalearner_experiment(
    df, metalearner="DR", quantile_regression=True, alpha=0.1, test_frac=0.1
):
...
def dr_cqr_random_forests(df, alpha):
    ...
    model = conformalMetalearner(
        alpha=alpha, base_learner="GBM", quantile_regression=quantile_regression, metalearner="DR"
    )
...
def ipw_cqr_random_forests(df, alpha):
    ...
    model = conformalMetalearner(
        alpha=alpha, base_learner="GBM", quantile_regression=True, metalearner="IPW"
    )
...
def x_cqr_random_forests(df, alpha):
    ...
    model = conformalMetalearner(
        alpha=alpha, base_learner="GBM", quantile_regression=True, metalearner="X"
    )
